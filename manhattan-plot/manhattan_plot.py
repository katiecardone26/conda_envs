...
class ManhattanPlot:
    ...
    DARK_CHR_COLOR = '#000000'
    LIGHT_CHR_COLOR = '#d3d3d3'
    NOVEL_HIT_COLOR = '#add8e6'
    NOVEL_TABLE_COLOR = '#add8e6'
    ...
    def __init__(self, file_path, test_rows=None, title='Manhattan Plot', subtitle = ''):
        """
    ...
    def load_data(self, delim='\s+'):
        ...
        if '.pickle' in self.path:
            ...
            self.df = pd.read_table(self.path, sep=delim, nrows=self.test_rows, dtype=str)
    ...
    def clean_data(self, col_map=None, logp=None, has_chr_prefix=False):
        ...
        self.df.dropna(subset=['P'],inplace=True)
        self.df['P'] = self.df['P'].apply(Decimal)
    ...
    def add_annotations(self, annot_df: pd.DataFrame, extra_cols=[]):
        ...
        self.df['ID_y'] = self.df['ID_y'].replace('-',np.nan)
        self.df['ID_x'].update(self.df['ID_y'])
        self.df = self.df.rename(columns={'ID_x': 'ID'})
        

    def update_plotting_parameters(self, log_p_axis_midpoint='',  annotate='', signal_color_col='', phewas_rep_color_col='', phewas_updown_col='', phewas_size_col='', phewas_annotate_col='', phewas_fill_col='', twas_color_col='', twas_updown_col='', sig='', sug='', annot_thresh='', ld_block='', vertical='', max_log_p='', invert='', merge_genes='', title='', subtitle=''):
        ...
        self.subtitle = self.__update_param(self.subtitle, subtitle)
    ...
    def get_thinned_data(self, log_p_round=2, additional_cols=[],thinning=False,CHR_POS_ROUND=''):
        ...
        if thinning:
            ...
            print('thinning false')
            self.thinned = self.df.copy()
            self.thinned['ROUNDED_X'] = self.thinned['ABS_POS'] // self.CHR_POS_ROUND * self.CHR_POS_ROUND
            self.thinned['ROUNDED_Y'] = pd.Series(-np.log10(self.thinned['P'].astype(float))).round(
                log_p_round)  # round to 2 decimals
            subset_cols = ['ROUNDED_X', 'ROUNDED_Y']
            subset_cols.extend(additional_cols)
            #self.thinned = self.thinned.sort_values(by='P').drop_duplicates(subset=subset_cols)
            print(len(self.thinned), 'Variants After Thinning')
    ...
    def plot_sig_signals(self, plot_sig=True, rep_genes=[], rep_boost=False, legend_loc=None):
        odds_df, evens_df = self.__find_signals_sig(plot_sig, rep_genes, rep_boost)
    ...
    def plot_annotations(self, plot_sig=True, rep_genes=[], rep_boost=False):
        ...
        annotDF = annotDF[full_mask]
        annotDF = annotDF.sort_values(by=['P'])
        annotDF = annotDF.set_index('ID')
    ...
    def plot_table(self, extra_cols={}, number_cols=[], rep_genes=[], keep_chr_pos=True, with_table_bg=True, with_table_grid=True, text_rep_colors=False, save=None):
        if self.vertical:
            self.__plot_table_vertical(extra_cols=extra_cols, number_cols=number_cols, rep_genes=rep_genes, keep_chr_pos=keep_chr_pos, save=save)
        else:
    ...
    def full_plot(self, rep_genes=[], extra_cols={}, number_cols=[], rep_boost=False, save=None, with_table=True,
                  ...
                  legend_loc=None, text_rep_colors=False, plot_sig=True, with_subtitle=True, width='', height=''):
        self.plot_data(with_table=with_table, legend_loc=legend_loc)
        self.plot_sig_signals(plot_sig=plot_sig, rep_genes=rep_genes, rep_boost=rep_boost, legend_loc=legend_loc)
        if with_table:
            ...
            self.plot_table(extra_cols=extra_cols, number_cols=number_cols, rep_genes=rep_genes, keep_chr_pos=keep_chr_pos, with_table_bg=with_table_bg, with_table_grid=with_table_grid, text_rep_colors=text_rep_colors, save=save)
        if with_title and with_subtitle:
        ...
        if save is not None:
            if width and height is not None:
                plt.savefig(save, width=width, height=height, dpi=save_res, bbox_inches='tight')
            else:
    ...
    def qq_plot(self, save=None, save_res=150, with_title=True, with_subtitle=True, steps=30):
    ...
    def __get_absolute_positions(self, active_df):
        ...
        #chr_lengths = pd.Series(CHR_LENGTHS)

        addMap = lastPos.cumsum()
        #addMap = chr_lengths.sort_index().cumsum()
        addMap[0] = 0
        addMap = addMap.sort_index()

        #active_df = active_df.sort_values(by=['#CHROM', 'POS'])
        ...
        #tickLocs = (chr_lengths.sort_index() / 2) + addMap.iloc[:-1].values
    ...
    def __find_signals_sig(self, plot_sig=True, rep_genes=[], rep_boost=False):
        odds_df, evens_df = self.__get_odds_evens()
    ...
    def __plot_table_vertical(self, extra_cols={}, number_cols=[], rep_genes=[], keep_chr_pos=True, save=None):
        ...
        annot_table['ID'] = annot_table['ID'].str.replace('_','\_')
        annot_table[number_cols] = annot_table[number_cols].applymap(lambda x: '{:.3}'.format(x))
        annot_table_dups = annot_table[['CHR', 'POS', 'ID']]
        annot_table_dups = annot_table_dups[annot_table_dups[['CHR', 'POS']].duplicated(keep = False)].sort_values(by = ['CHR', 'POS'])
        if len(annot_table_dups.index) > 0:
            print('duplicate annotations present')
            annot_table_dups = annot_table_dups.drop_duplicates()
            annot_table_dups.to_csv(save.replace('.png', '.duplicate_annotations.csv'), index = None)
        ...
        for i in range(len(annot_table)):
            ...
            cell_text = cell_text.replace('$\it{','')
